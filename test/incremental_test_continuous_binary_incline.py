...
bo = 16  # 2 5 9 13 15 16
seed = bo  # 25 28 0 1 3 4
rs = bo
times = 6
lam = 0.95
ki = False
...
class ContinuousDataLoaders:
    def __init__(self, data_copy, t, test=True, kick_imbalance=False):
        ...
        self.neg_start = 0
        self.t = 0
        self.pos_end = round(0.5 * self.n_pos)
        self.neg_end = round(0.2 * self.n_neg)
    ...
    def next_loader(self):
        ...
        self.pos_end += round(0.1 * self.n_pos)
        self.neg_end += round(0.16 * self.n_neg)
        if self.t == self.times:
...
np.savetxt('{}continuous_incremental/incline/seed{}_rs{}_lam{}{}.txt'.format(arguments.rec_path, seed, rs, lam,
                                                                     '_ki' if ki else ''), fs)
np.savetxt('{}continuous_incremental/incline/prf_seed{}_rs{}_lam{}{}.txt'.format(arguments.rec_path, seed, rs, lam,
                                                                     '_ki' if ki else ''), results, fmt='%.3f',
           delimiter='\t')
print('Time Elapsed:{:.2f} min'.format((time_end - time_start) / 60))
plot(fs, 'Incline', None)
